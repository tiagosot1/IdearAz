...
class Supermodel extends CI_Model {
	...
	*/
	...
	{
		return $this->db->from('configuracao')
						->where('id', 1)
						->limit(1)
						->get()
						->row();
	...
	*/
	...
	public function get_table($table=false, $id=false)
	{
	...
	public function get_table_full ($table= false, $where1 = false,  $data = false, $where2 = false, $id2 = false, $join_table =false, $join_forenkey=false)
	{
		$id1 = $data[ 'subdominio' ][ 0];
		$this->db->from($table);
		$this->db->where($where1, $id1);
			...
			if($join_table !='' and $join_forenkey !='')
			{
				$this->db->join($join_table, $join_forenkey);
			}
	...
	public function update_table($table= false, $columns = array(), $where1 = false, $id1 = false,$where2 = false, $id2 = false)
	{
	...
	public function delete_table($table=false, $name_campo=false, $id=false)
	{
		if( $this->db->delete($table, array($name_campo => $id) ) )
			return TRUE;
		else
			return FALSE;
	}
