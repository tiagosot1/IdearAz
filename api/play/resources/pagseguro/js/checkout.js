...
MyApplication.CheckoutPage = new function() {
	...
	var base_url = window.location.protocol + "//"+window.location.host + "/user/";
	...
	var updateSessionId = function(callback) {
		...
		$.ajax({
			//url: "http://localhost/pag1/sessionid.php",
			url: base_url + "payment/Sessao_pagamento/1",
			type:"GET",
			cache: false,
	...
	var doPayment = function(params, callback) {
		...
		$.ajax({
			...
			//url: "http://localhost/pag1/payment.php",
			url: base_url + "payment/Sessao_pagamento/2",
			data: params,
			dataType: 'json',
			cache: false,
			success: function(response) {
	...
	// Pagamento com cartão de crédito
		...
		updateCardToken(function(cardToken) {
			...
			doPayment(params, function(transaction){
				...
				$.ajax({
				  url: base_url + 'payment/transacao_pagseguro/'+transaction.code,
				  success: function(response){
					  if(response == "ok"){
					 	 showWaitingPayment("Transação Com Sucesso!!!");	 
					  }
					  else{
						 updateCardToken() 
					  }
	...
	/*var boletoEvents = function() {
		...
		$("#boletoButton").click(function(){
			...
			doPayment(params, function(transaction){
				...
				$.ajax({
				  url: 'http://playcash.com.br/payment/transacao_pagseguro/'+transaction.code,
				 });
			});
		});
